...
- `PAYMENT_SETTLED` - One or more merchant payouts completed, funds sent to payout account. A single event bundles every settled payout and every payment they contain (each payment carries its `payoutId`)
...
The `PAYMENT_SETTLED` event has a different payload structure. A single event bundles **every** settled payout and **every** payment they contain. When multiple payouts are marked as paid at once (bulk action), you will still receive only one webhook — each payment references its originating payout through `payoutId`:
